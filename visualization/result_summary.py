...
def triangle_plot_per_seed(dir_path, p_id, payoff_array):
    ...
    for i_s in range(2):
        if payoff_array.shape[i_s] == 3 and equilibrium_points:
            ...
            triangle_plot(df_state.values, "{}/figure/seed_{}_time_average_trajectories_triangle_{}".format(dir_path, p_id, i_s), "Time Average Trajectory", equilibrium_points[i_s], lines_end_point)
            ...
            triangle_plot(df_state_time_average.values, "{}/figure/seed_{}_trajectories_triangle_{}".format(dir_path, p_id, i_s), "Trajectory", equilibrium_points[i_s], lines_end_point)
...
def final_summary(dir_path, n_p):
    ...
    for seed in range(n_p):
        try:
            ...
            time_average_exploitability_file = glob.glob('{}/csv/seed_{}_time_average_exploitability.csv'.format(dir_path, seed))[0]
            ...
            final_summary_dict['time_average_exploitability'].append(time_average_exploitability_df.tail(1).values[0][0])
    ...
    plt.savefig("{}/figure/exploitability_summary.png".format(dir_path))
    plt.clf()
    plt.close()
